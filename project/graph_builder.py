...
class GraphBuilder:
    ...
    def get_name(self, x):
        if not isinstance(x, tinytorch.Scalar) and not isinstance(x, tinytorch.Tensor):
            return "constant %s" % (x,)
        elif len(x.name) > 15:
        ...
        while queue:
            ...
            if cur.is_constant() or cur.is_leaf():
                ...
                for input in cur.history.inputs:
                    if not isinstance(input, tinytorch.Scalar) and not isinstance(
                        input, tinytorch.Tensor
                    ):
                        continue
                    queue.append([input])
        return G
